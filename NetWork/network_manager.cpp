...
// Simple thread-safe network logger (低頻度のみログ出す)
...
// start_as_host/start_as_client はソケット初期化後にワーカーを起動する
...
// discover_and_join の動作は変えず（ブロック型探索）だが、探査用の待ち時間を短縮
...
// update: ワーカーが受け取りキューに積んだパケットを最大 m_maxPacketsPerFrame 件処理する（非常に軽量）
...
// send_state_to_clients_round_robin: 1クライアント分だけを送って負荷を分散（ワーカーから呼ぶ）
...
// check_existing_firewall_rule: 最小限にしておく
bool NetworkManager::check_existing_firewall_rule() {
    // コストの高いCOM列挙は避けて false を返しておく（add_firewall_exception では管理者判定だけで終了）
    return false;
}

// add_firewall_exception: 管理者有無だけチェックして重い操作はしない
bool NetworkManager::add_firewall_exception() {
    // 管理者チェックを行うが、実際にCOMで追加する処理は行わない（学校環境では多くの場合不可）
    BOOL isAdmin = FALSE;
...
// handle_channel_scan / handle_channel_info: 軽量に保持
...
void NetworkManager::send_state_to_all(std::vector<GameObject*>& worldObjects) {
    ...
    // ホストプレイヤー（ID=1）を必ずローカルプレイヤーから取得
    GameObject* localPlayer = nullptr;
    // main.cppのGetLocalPlayerGameObject()を使用
